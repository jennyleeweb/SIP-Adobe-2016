...
class Scroller(object):
    ...
    # def draw_sky(self):
    #     pygame.draw.circle(screen, (255, 186, 105), [40, 50], 0)

    def add_buildings(self):
        # combined_width= 0
        # for building1 in self.buildings_list:
        ...
        # scroller.
        # """
        #--need width of scroller, width of all buildings created
            #until buildings fill scroller
                #call add_building

    def add_building(self, x_location):
        width= random.randint(10, 70)
        # building= Building(x_location, random.randint(0, 600), width, random_height(), GREY)
        self.buildings_list.append(Building(x_location, 600, width, random_height(), self.color))
    ...
    def move_buildings(self):
        width= random.randint(10, 70)
        # x_location= current_width
        last_building= self.buildings_list[-1]
        first_building= self.buildings_list[0]
        for building1 in self.buildings_list:
            building1.move(self.speed)   
        if last_building.width >= 800:
            self.buildings_list.remove(building1)
        if first_building.width <= 0:
            self.buildings_list.insert(0, self.buildings_list[0].width)
