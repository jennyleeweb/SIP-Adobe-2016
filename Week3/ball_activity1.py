...
# Ball 2
xpos= int(SCREEN_WIDTH/4)
ypos= int(SCREEN_HEIGHT/4)

speedx=  random.randint(-5, 5)
speedy = random.randint(-5, 5)

size_ball= random.randint(5, 25)
...
while not done:
    ...
    # Ball 2
    pygame.draw.circle(screen, ball_color, [xpos, ypos], size_ball)

    if xpos >= SCREEN_WIDTH - size_ball or xpos < size_ball:
        speedx = speedx * -1

    if ypos >= SCREEN_HEIGHT - size_ball or ypos < size_ball:
        speedy = speedy * -1
